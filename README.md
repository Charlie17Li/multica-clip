...
4. 在任意 `http` / `https` 文章页打开扩展。弹窗默认显示当前站点匹配的项目和 Agent；如有需要，可仅对此次采集临时调整，再点击“创建采集 issue”。页面区域快照以 Side Panel 为主流程，见下文。
...
勾选“我确认要提取并上传页面文字快照”后，才可以点击“添加页面区域”。弹窗会将已确认的草稿交给与当前标签页关联的 Side Panel；Side Panel 会保持打开，方便你在页面上移动鼠标选择元素、单击或 Enter 确认，使用 `Alt` + `↑` 选择父元素，或按 Esc 取消。可重复“添加页面区域”建立有序列表；重复选择会忽略，后选父区域会替换此前已选的子区域。可逐项移除区域，或点击“使用整页快照”清空列表。

选择阶段扩展仅在会话存储中保存 URL 与临时 DOM 定位描述；不会读取、保存或上传正文。只有点击“创建采集 issue”后才会按顺序重新定位区域、运行 adapter 清洗规则并提取纯文本。若页面重渲染或导航造成任一定位失效，创建会被阻断，绝不会改用整页快照；请移除或重新选择失效区域、显式清空列表后选择整页快照，或取消确认改为仅链接模式。

若 Side Panel 不可用（包括不支持的浏览器环境），弹窗会提示并回退到原有的页面内选择器；选定后请重新打开弹窗。区域选择仅适用于常规 `http(s)` 页面；浏览器受保护页面、跨站 iframe 与封闭 Shadow DOM 等边界无法选择。
