...
发生错误时，弹窗或设置页会显示“复制诊断报告”。请先确认页面提示（常见原因是服务器权限、网络、令牌或目标配置），然后复制报告并附到 Multica issue 或支持请求中。报告只包含最近 20 条时间戳、扩展版本、浏览器信息、事件类别、HTTP 状态和服务器来源；**不包含**访问令牌、备注、正文快照或完整页面 URL。
