...
勾选“我确认要提取并上传页面文字快照”后，才可以点击“选择页面区域”。弹窗会关闭并回到页面：移动鼠标选择元素，单击或 Enter 确认，`Alt` + `↑` 可选择父元素，Esc 取消。此时扩展仅在会话存储中保存 URL 与临时 DOM 定位描述；不会读取、保存或上传正文。重新打开弹窗并点击“创建采集 issue”时，才会重新定位该区域、运行既有 adapter 清洗规则并提取纯文本。

区域定位可能因页面重渲染而失效；此时不会上传错误内容，扩展会提示你重新选择、使用整页快照，或改为仅链接模式。区域选择仅适用于常规 `http(s)` 页面；浏览器受保护页面、跨站 iframe 与封闭 Shadow DOM 等边界无法选择。
