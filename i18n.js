const translations = {
  en: {
    ...
    selectRegion: "Add page region", selectRegionHint: "Choose an element on the page. This only saves a temporary location, not page text.",
    regionSelected: "{count} page region(s) selected. They will be read only when you create the capture.", removeRegion: "Remove", useWholePage: "Use whole-page snapshot",
    regionInvalid: "One or more selected page regions are no longer available. Remove or re-select them before creating the capture.",
    regionPickerUnavailable: "This page does not allow region selection. Try a regular http(s) page outside a cross-site frame.",
    sidePanelUnavailable: "Side Panel is unavailable, so the picker will continue on this page. Reopen the popup after selecting a region.",
    ...
    created: "Created {reference}{fallback}{warning}", snapshotFallback: " in link mode because the snapshot could not be extracted.", adapterFallback: " A site-specific extraction was unavailable; the capture includes the generic result and a warning.",
  ...
  "zh-CN": {
    ...
    selectRegion: "添加页面区域", selectRegionHint: "在页面中选择一个元素；此操作只保存临时位置，不读取页面文字。",
    regionSelected: "已选择 {count} 个页面区域；只有在创建采集时才会读取它们。", removeRegion: "移除", useWholePage: "使用整页快照",
    regionInvalid: "一个或多个已选页面区域已失效。请先移除或重新选择后再创建采集。",
    regionPickerUnavailable: "此页面不允许选择区域。请在常规 http(s) 页面中重试，并避开跨站框架。",
    sidePanelUnavailable: "Side Panel 不可用，将在当前页面继续选择。选定区域后请重新打开弹窗。",
    ...
    created: "已创建 {reference}{fallback}{warning}", snapshotFallback: "；因无法提取快照，已使用仅链接模式。", adapterFallback: "；站点专用提取不可用，已使用通用结果并记录警告。",
